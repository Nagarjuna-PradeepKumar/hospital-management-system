...
router.post('/login',async(req,res)=>{
    ...
    const usernameexist =await user.findOne({"user_name" : req.body.user_name,"phoneno":req.body.phoneno,"status":true})
...
router.post('/getdata',checkadmin,async(req,res)=>{
    ...
    if(req.body.user_name!=undefined && req.body.phoneno!=undefined){
        const datas = await user.find({"user_name":req.body.user_name,"phoneno":req.body.phoneno,"status":true})
...
router.post('/changedata',checkadmin,async(req,res)=>{
    ...
    if(checkadmin.length>=2){return res.send({"error":"Cannot create more than 2 administrators"})}
        ...
        const usernameexist =await user.findOne({"_id":{'$ne':req.body.user_id},"user_name" : req.body.user_name,"phoneno":req.body.phoneno,"status":true})
        if(!usernameexist){
            try{
                const newdata= await user.findOneAndUpdate({"_id":req.body.user_id,"status":true},{
                    "user_name":req.body.user_name,
                    "user_role":req.body.user_role,
                    "phoneno":req.body.phoneno,
                    "email":req.body.email,
                ...
                const modattendence =await attendence.findOneAndUpdate({"user_id":req.body.user_id},{"user_name":req.body.user_name,"user_role":req.body.user_role,"phoneno":req.body.phoneno},{useFindAndModify:false,new:true})
                if(await newdata && await modattendence){return res.send({"success":"updated successfully","newdata":newdata})}else{/* redirect to error page */ return res.send({"error":"Error saving to database"})}
...
router.post('/verify',async(req,res)=>{     
    try{
        ...
        try{const userdata =await user.findOne({"_id":await payload.id,"role":await payload.role,"status":true})
            if(userdata)return res.send({"success":"Found user","name":userdata.user_name,"role":userdata.user_role,"id":userdata._id})
        }catch(err){return res.send({"error":"Could not find any user"})}
    }catch(err){return res.send({"error":"Session expired, Login again to continue"})}
})

router.post('/getdoctor',async(req,res)=>{   
    try{
    const list=[];
    const datas = await user.find({"status":true,"user_role":"doctor"},{"user_name":1,"_id":0})
