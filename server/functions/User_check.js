...
/*------------redirect to error page--------------------- */
        try{const userdata =await user.findOne({"_id":await payload.id,"role":await payload.role,"status":true})
            if((admin.findIndex(x=>x===userdata.user_role)===-1))return res.send({"error":"g"})
        }catch(err){return /*redirect to error page*/res.send({"error":"d"})}
...
/*------------redirect to error page--------------------- */
    ...
    }catch(err){return res.send({"error":"f"})}
/*------------redirect to error page--------------------- */   
    next()
}
